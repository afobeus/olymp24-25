...
for case, count in TEST_CASES:
    for i in range(count):
        with open(fr"tests/{test_count}", 'w') as file:
            file.write(get_test_string(*case.gen_case_common()))

        os.system(f"{COMPILER_PATH}/task_{CURRENT_TASK}.exe < tests/{test_count} > temp_cpp.a")
        os.system(f"python solutions/sol_{CURRENT_TASK}.py < tests/{test_count} > temp_py.a")
        ...
        if cpp_ans.split() != py_ans.split():
            ...
            with open(f"tests/{test_count}") as file:
                print(file.read())
            print("py_ans:")
            print(py_ans)
            print("cpp_ans:")
            ...
            with open(f"tests/{test_count}.a", 'w') as file:
                file.write(cpp_ans)
